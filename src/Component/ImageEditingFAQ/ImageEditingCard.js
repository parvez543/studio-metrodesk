...
const ImageEditingCard = ({i}) => {
    const [item, setItem] = useState(false)
    return (
        <div className=''>
             <div className='flex items-center justify-between accordion py-4'>
                <h2 className='font-medium text-xl cursor-pointer' onClick={() => setItem(!false)}>{i.questions}</h2>
            ...
            {
                item && <div>
                    <p className='text-gray-500 text-xl py-6 text-left'>{i.answer}</p>
                </div>
            }
        </div>
...
export default ImageEditingCard;